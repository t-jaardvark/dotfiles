...
# TIMER (requires termdown package)
function tmr
    ...
    termdown $argv
    set termdown_status $status

    # Only play the sound if termdown was successful
    if test $termdown_status -eq 0
...
alias .4='cd ../../../..'
alias .5='cd ../../../../..'
alias ei='micro $HOME/git/tjsh/install/arch-postinstall '
alias ep="micro $HOME/.config/fish/config.fish && source $HOME/.config/fish/config.fish"
# vim and emacs
alias vim='nvim'
alias emacs="emacsclient -c -a 'emacs'"
alias em='/usr/bin/emacs -nw'
alias rem="killall emacs || echo 'Emacs server not running'; /usr/bin/emacs --daemon" # Kill Emacs and restart daemon..
alias mirrors="sudo reflector --country 'United States,Canada' --age 6 --protocol https --sort rate --save /etc/pacman.d/mirrorlist --number 5"
...
function xed
    if test -n "$argv[1]"
        command xed "$argv[1]" > /dev/null 2>&1 &
    else
        echo "Usage: xed <filename>"
    end
end
